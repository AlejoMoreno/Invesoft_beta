...
$fechas = array();
$i=0;
$bill_usuarios = mysqli_query($conexion,"SELECT COUNT( * ) AS cantidad,  `iduser` ,  `historial`.`fecha` ,  `usuarios`.`nombre` FROM  `historial` INNER JOIN  `usuarios` ON  `historial`.`iduser` =  `usuarios`.`ncedula` GROUP BY  `historial`.`fecha` ") or die(mysqli_error($conexion));
...
$usuarios = array();
$i=0;
$bill_usuarios = mysqli_query($conexion,"SELECT COUNT( * ) AS cantidad,  `iduser` ,  `historial`.`fecha` ,  `usuarios`.`nombre` FROM  `historial` INNER JOIN  `usuarios` ON  `historial`.`iduser` =  `usuarios`.`ncedula` GROUP BY  `historial`.`iduser` ") or die(mysqli_error($conexion));
while ($row = $bill_usuarios->fetch_assoc()) {
	$usuarios[$i] = $row;
    $i++;
}
...
$(function () { 
    ...
    $('#container').highcharts({
        title: {
            text: 'Historial de ingreso por usuario',
            x: -20 //center
        },
        subtitle: {
        ...
        series: [
            ...
            for ($i=0; $i < sizeof($usuarios); $i++) { ?>
            	{ name: "<?php echo $usuarios[$i]['nombre']; ?>", 
            	  data: [
            	  <?php 
            	  	for ($j=0; $j < sizeof($fechas); $j++) { 
            	  		if($usuarios[$i]['iduser'] == $fechas[$j]['iduser']){
            	  			?>
		            	<?php echo $fechas[$j]['cantidad']; ?>, <?php 
		            	}
		            }
		            ?>
		          0]},
...
<table class="tg">
    <tr>
        <td class="tg-g4jy">Usuarios</td>
        <td class="tg-g4jy">Cantidad</td>
        <td class="tg-g4jy">Fecha</td>
    </tr>';
    for ($i=0; $i < sizeof($fechas); $i++) { 
        echo '
        <tr>
            <td class="tg-iacq">'.$fechas[$i]['nombre'].'</td>
            <td class="tg-iacq">'.$fechas[$i]['cantidad'].'</td>
            <td class="tg-iacq">'.$fechas[$i]['fecha'].'</td>
        </tr>';
    }
