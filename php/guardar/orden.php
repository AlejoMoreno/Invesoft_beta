...
$consecutivo = 'O'.$_POST['idremision'];
...
echo 'Orden pedido completada<br>';
...
function SalidaInventario(){
	...
	for ($i=1; $i <= $trs; $i++) { 
		...
		$sqlSalida ="INSERT INTO  `osmed`.`salidadeinventario` VALUES (null,  '$idinventario',  '$consecutivo',  '$cantidad',  '0',  'ORDEN', '$lote');";
		mysqli_query($conexion, $sqlSalida) or die(mysqli_error($conexion));
		$fechacreacion = date('Y-m-d');
		$horacreacion = date('H:i:s');
		registroKardex($idinventario,  $consecutivo,  $fechacreacion,  $horacreacion,  $_SESSION['bodegaactual'],  $_POST['nit_cliente'],  $_SESSION['id'],  $cantidad,  '0',  $costo,  $_POST['nit_cliente'],  $saldoanterior,  $cantidad,  $cantidad,  ($saldoanterior-($cantidad-$cantidad)),  'ORDEN',  '-');
	}
}
