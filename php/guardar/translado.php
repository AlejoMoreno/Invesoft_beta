...
$consecutivo = 'T'.$_POST['idremision'];
...
VALUES (
null,  '$idsalida',  '$bodegaactual',  '$idpaciente',  '$ns',  '$idclientes',  '$fecha_remision',  '0',  '0',  '0',  '0',  '1',  '0', '0', '$consecutivo', '$observaciones');";
...
function SalidaInventario(){
	...
	for ($i=1; $i <= $trs; $i++) { 
		...
		$sqlSalida ="INSERT INTO  `osmed`.`salidadeinventario` VALUES (null,  '$idinventario',  '$consecutivo',  '$cantidad',  '0',  'REMISION', '$lote');";
		descontarInventario($idinventario,$cantidad,$_POST['nit_cliente']);
		mysqli_query($conexion, $sqlSalida) or die(mysqli_error($conexion));
		//registro en el kardex por producto
		$fechacreacion = date('Y-m-d');
		$horacreacion = date('H:i:s');
		registroKardex($idinventario,  $consecutivo,  $fechacreacion,  $horacreacion,  $_SESSION['bodegaactual'],  $_POST['nit_cliente'],  $_SESSION['id'],  $cantidad,  0,  $costo,  $_POST['nit_cliente'],  $saldoanterior,  '0',  $cantidad,  ($saldoanterior-$cantidad),  'REMISION',  '-');
		//otherbodega($idinventario,$_POST['nit_cliente']);
	}
}
...
function descontarInventario($idinventario,$cantidad,$clientebodega){
    ...
    echo '<br>Usted se ha actualizado correctamente.'; 
}

function otherbodega($idinventario,$bodega){
	include_once "../conexion.php";
	$conexion= conexion();
	$bill_salida = mysqli_query($conexion,"INSERT INTO `inventario` (`idbodega` ,`ncedula` ,`idmarcas` ,`idlineas` ,`referencia` ,`descripcion` ,`numerolote` ,`tipopresentacion` ,`stockminimo` ,`stockmaximo` ,`precio` ,`costo` ,`estado` ,`saldo` ,`fechadevencimiento` ,`iva` ,`fechamodificado`) SELECT `idbodega` ,`ncedula` ,`idmarcas` ,`idlineas` ,`referencia` ,`descripcion` ,`numerolote` ,`tipopresentacion` ,`stockminimo` ,`stockmaximo` ,`precio` ,`costo` ,`estado` ,`saldo` ,`fechadevencimiento` ,`iva` ,`fechamodificado` FROM  `inventario` WHERE `idinventario` LIKE '$idinventario' ") or die(mysqli_error($conexion));
	
	$sql = "UPDATE  `osmed`.`inventario` 
	   SET  `idbodega` =  '$bodega' 
	   WHERE `inventario`.`idinventario` = '$idinventario'";
   
    mysqli_query($conexion, $sql) or die(mysqli_error($conexion));
    echo '<br>Usted se Transladado producto correctamente.'; 	
