...
$bill_remision = mysqli_query($conexion,"SELECT * FROM  `remision` WHERE  `documento` LIKE  'O%' ORDER BY  `remision`.`idremision` DESC ") or die(mysqli_error($conexion));
$rowremision = $bill_remision->fetch_assoc();
$documento = $rowremision['documento'];
$numdoc = substr($documento, 1)+1;

//consulta all productos
$bill_producto = mysqli_query($conexion,"SELECT *, sum(saldo) as saldos FROM  `inventario` GROUP BY `referencia` ORDER BY  `inventario`.`descripcion` ASC ") or die(mysqli_error($conexion)); 
...
<br>
<center><h2>Orden O-</h2></center><hr>
<table class="tg">
  <tr>
    <th class="tg-031e" colspan="5">
		<div id="numeroremision"><label>Número Orden: </label><?php echo '<input type="text" name="idremision" id="idremision" value="'.$numdoc.'" disabled>';?></div>
    </th>
  </tr>
  <tr>
    <td class="tg-le8v" colspan="2">CLIENTE :</td>
    <td class="tg-le8v" colspan="3">FECHA ORDEN :</td>
  </tr>
  ...
  <tr>
    <td class="tg-yw4l" colspan="4" rowspan="2"><label>DATOS DEL PACIENTE :</label>
    	<table class="datos">
    		<tr>
    			<td><label>CEDULA :</label><input name="datosPaCed" id="datosPaCed" placeholder="Cedula"></td>
    			<td><label>NOMBRE PACIENTE :</label><input name="datosPaNPa" id="datosPaNPa" placeholder="Nombre Paciente"></td>
    			<td><label>NOMBRE CIRUJANO :</label><input name="datosPaNCi" id="datosPaNCi" placeholder="Nombre Cirujano"></td>
    		</tr>
    		<tr>
    			<td><label>HISTORIA CLINICA :</label> <input name="datosPaHis" id="datosPaHis" placeholder="Historia Clinica"></td>
    			<td><label>FECHA :</label><input type="date" name="datosPaFec" id="datosPaFec" placeholder="Fecha Cirujia"></td>
    			<td></td>
    		</tr>
    	</table>
    </td>
...
<br><br>
<label>Condiciones de este Pedido :</label><br><textarea id="observaciones" placeholder="Escriba las observaciones de este documento"></textarea>
...
$('#mas').click(function(){
	...
	for(i=0;i<obj.length;i++){
		producto = producto+'<option value="'+obj[i].referencia+'">'+obj[i].descripcion+' {LOTE: '+obj[i].numerolote+', SALDO: '+obj[i].saldo+', PRECIO: '+obj[i].precio+', COSTO: '+obj[i].costo+'}</option>';
	}
	$('#productos').append('<tr calss="selected" id="fila'+cont+'" onclick="seleccionar(this.id)"><td><input name="ref" id="ref'+trs+'" ></td><td><select name="nom'+trs+'" id="nom'+trs+'"><option value="1">SELECCIONE PRODUCTO</option>'+producto+'</select></td><td width="10%"><input type="hidden" name="lot" id="lot'+trs+'"></td><td  width="10%"><input name="cant" id="cant'+trs+'"></td><td  width="10%"><input type="text" name="precio" id="precio'+trs+'"></td></tr>');	
...
function gruardarRemision(){
	...
	if(productos == '' || idremision == '' || fecha_remision == '' || nit_cliente == '' || vendedor == '' || datosPaCed == '' || datosPaNPa == '' || datosPaNCi == '' || datosPaHis == '' || datosPaFec == '' || observaciones == ''){
