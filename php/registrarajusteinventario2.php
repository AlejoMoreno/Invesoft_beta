...
$bill_remision = mysqli_query($conexion,"SELECT * FROM  `remision` WHERE  `documento` LIKE  'S%' ORDER BY  `remision`.`idremision` DESC ") or die(mysqli_error($conexion));
$rowremision = $bill_remision->fetch_assoc();
$documento = $rowremision['documento'];
$numdoc = substr($documento, 1)+1;

//consulta all productos
$bill_producto = mysqli_query($conexion,"SELECT *, saldo as saldos FROM  `inventario` WHERE  `idbodega` = '$bodegaactual' ORDER BY  `inventario`.`descripcion` ASC ") or die(mysqli_error($conexion)); 
...
<br>
<center><h2>AJUSTE INVENTARIO SALIDA-S</h2></center><hr>
<table class="tg">
  <tr>
    <th class="tg-031e" colspan="5">
		<div id="numeroremision"><label>AJUSTE INVENTARIO SALIDA: </label><?php echo '<input type="text" name="idremision" id="idremision" value="'.$numdoc.'" disabled>';?></div>
    </th>
  </tr>
  <tr>
    <td class="tg-le8v" colspan="2">TERCERO :</td>
    <td class="tg-le8v" colspan="3">FECHA AJUSTE :</td>
  </tr>
  <tr>
  ...
  <tr>
    <td class="tg-yw4l" colspan="2">NIT O CEDULA<input type="text" name="nit_cliente" id="nit_cliente" onblur="buscarCedula('nit_cliente')" placeholder="Nit / Cedula ej.(1 030570356)"></td>
  ...
  <tr>
    <td class="tg-le8v" colspan="4" rowspan="2"><label>DATOS DEL PACIENTE :</label>
    	<table class="datos">
    		<tr>
    			<td><label>CEDULA :</label><input type="hidden" name="datosPaCed" id="datosPaCed" placeholder="Cedula" value="2016"></td>
    			<td><label>NOMBRE PACIENTE :</label><input type="hidden" name="datosPaNPa" id="datosPaNPa" placeholder="Nombre Paciente" value="2016"></td>
    			<td><label>NOMBRE CIRUJANO :</label><input type="hidden" name="datosPaNCi" id="datosPaNCi" placeholder="Nombre Cirujano" value="2016"></td>
    		</tr>
    		<tr>
    			<td><label>HISTORIA CLINICA :</label><input type="hidden" name="datosPaHis" id="datosPaHis" placeholder="Historia Clinica" value="2016"></td>
    			<td><label>FECHA CIRUGIA :</label><input type="hidden" name="datosPaFec" id="datosPaFec" placeholder="Fecha Cirujia" value="2016"></td>
    			<td></td>
    		</tr>
    	</table>
    </td>
...
function gruardarRemision(){
    ...
    $.ajax({
            data:  parametros,
            url:   '../php/guardar/ajusteinventario2.php',
            type:  'post',
            beforeSend: function () {
