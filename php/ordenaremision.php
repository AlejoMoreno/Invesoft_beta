...
<table class="tg">
  ...
  <tr>
    <td class="tg-yw4l" colspan="2">NIT O CEDULA :<input type="text" value="'.$cedulatercero.'" name="nit_cliente" id="nit_cliente" onblur="buscarCedula(`nit_cliente`)" placeholder="Nit / cedula ej.(1 030570356)" disabled></td>
  ...
  <tr>
    <td class="tg-yw4l" colspan="2">DIRECCION :<input type="text" value="'.$direccion_cliente.'" name="direccion" id="direccion" onkeyup="uppercase(`direccion`)" placeholder="ej.(Direccion Autopista medellin)" disabled></td>
    <td class="tg-yw4l" colspan="2" rowspan="2"><label>DATOS DEL PACIENTE</label>
    	<table class="datos">
    		<tr>
    			<td>CEDULA: <input name="datosPaCed" value="'.$cedula.'" id="datosPaCed" placeholder="Cedula" disabled></td>
    			<td>NOMBRE PACIENTE : <input name="datosPaNPa" value="'.$nombrepaciente.'" id="datosPaNPa" placeholder="Nombre Paciente" disabled></td>
    			<td>NOMBRE CIRUJANO :<input name="datosPaNCi" value="'.$nombrecirujano.'" id="datosPaNCi" placeholder="Nombre Cirujano" disabled></td>
    		</tr>
    		<tr>
    			<td>HISTORIA MEDICA :<input name="datosPaHis" value="'.$historiaclinica.'" id="datosPaHis" placeholder="Historia Clinica" disabled></td>
...
<table id="productos">
	<tr>
		<td>REFERENCIA</td>
		<td>NOMBRE</td>
		<td>CANTIDAD ORDEN</td>
		<td>CANTIDAD A REMISIONAR</td>
		<td>PRECIO</td>
...
$('#mas').click(function(){
	...
	$('#productos tr:last').after('<tr calss="selected" id="fila'+trs+'" onclick="seleccionar(this.id)"><td><input name="ref" id="ref'+trs+'" ></td><td><input name="nom" id="nom'+trs+'"></td><td><input name="cant" id="cant'+trs+'"></td><td><input name="cantnueva'+trs+'" id="cantnueva'+trs+'" value="0"></td><td><input name="precio" id="precio'+trs+'"></td></tr>');	
});
...
function gruardarRemision(){
    ...
    $.ajax({
            data:  parametros,
            url:   '../php/guardar/ordenaremision.php',
            type:  'post',
            beforeSend: function () {
