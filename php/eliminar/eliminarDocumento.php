...
$bill_cliente = mysqli_query($conexion,"SELECT * FROM  `terceros` WHERE tipo not like 'BODEGA'") or die(mysqli_error($conexion)); 
...
if(isset($_POST['beliminacompra'])){
	...
	if($documento == '' or $tercero == ''){
		...
		elseif ($idtercero != $idtercero1) {
			...
			$sql = "DELETE FROM `salidadeinventario` WHERE  `idkit` LIKE  '$documento';";
			...
			echo "<script type='text/javascript'>
				window.open('../eliminar/eliminarDocumento.php', 'MsgWindow', 'width=800%, height=600');
			</script>";
		}
	}
}
...
<form action="" method="post">
	...
	<input type="submit" name="beliminacompra" id="boton" value="Eliminar">
</form>
