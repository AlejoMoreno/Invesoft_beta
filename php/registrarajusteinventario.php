...
$bill_cliente = mysqli_query($conexion,"SELECT * FROM  `terceros` where `tipo` like 'TERCERO'") or die(mysqli_error($conexion));         


//consulta el ultimo registro remision
$bill_remision = mysqli_query($conexion,"SELECT * FROM  `remision` WHERE  `documento` LIKE  'E%' ORDER BY  `remision`.`idremision` DESC ") or die(mysqli_error($conexion));
...
$bill_producto = mysqli_query($conexion,"SELECT *,saldo as saldos FROM  `inventario` WHERE  `idbodega` = 1 ORDER BY  `inventario`.`descripcion` ASC ") or die(mysqli_error($conexion)); 
...
//bill kit consulta
$bill_kit = mysqli_query($conexion,"SELECT * FROM  `salidadeinventario` WHERE  `tipo` LIKE '%%' GROUP BY `idkit` ") or die(mysqli_error($conexion)); 

$bill_kits = mysqli_query($conexion,"SELECT * FROM  `salidadeinventario` WHERE `tipo` LIKE '%%' ORDER BY `idkit` ") or die(mysqli_error($conexion)); 
$rawdata = array(); //creamos un array
...
<br>
<center><h2>AJUSTE INVENTARIO ENTRADA-E</h2></center><hr>
<table class="tg">
  <tr>
    <th class="tg-031e" colspan="5">
		<div id="numeroremision"><label>AJUSTE INVENTARIO ENTRADA: </label><input type="text" name="idremision" id="idremision"  VALUE="<?php echo $numdoc?>" disabled></div>
    </th>
  </tr>
  <tr>
    <td class="tg-le8v" colspan="2">TERCERO</td>
  ...
  <tr>
    <td class="tg-yw4l" colspan="2"><label>NIT O CEDULA</label><input type="text" name="nit_cliente" id="nit_cliente" onblur="buscarCedula('nit_cliente')" placeholder="Nit / Cedula ej.(1 030570356)"></td>
    <td class="tg-yw4l" width="20%"><label>USUARIO</label><input type="text" name="vendedor" id="vendedor" placeholder="vendedor" value="<?php echo $ns;?>" disabled></td>
  </tr>
  <div id="resultado"></div>
  <tr>
    <td class="tg-le8v" colspan="4" rowspan="2"><label>DATOS DEL PACIENTE</label>
    	<table class="datos">
    		<tr>
    			<td><input type="hidden" name="datosPaCed" id="datosPaCed" value="1030570356" placeholder="Cedula"></td>
    			<td><input type="hidden" name="datosPaNPa" id="datosPaNPa" value="1030570356" placeholder="Nombre Paciente"></td>
    			<td><input type="hidden" name="datosPaNCi" id="datosPaNCi" value="1030570356" placeholder="Nombre Cirujano"></td>
    		</tr>
    		<tr>
    			<td><input type="hidden" name="datosPaHis" id="datosPaHis" value="1030570356" placeholder="Historia Clinica"></td>
    			<td><input type="hidden" name="datosPaFec" value="1030570356" id="datosPaFec" placeholder="Fecha Cirujia"></td>
    			<td></td>
    		</tr>
    	</table>
    </td>
...
<table>
	<tr>
		<td>SubTotal: <input type="hidden" name="bruto" id="bruto"></td>
		<td>Iva: <input type="hidden" name="iva" id="iva" value="0"></td>
		<td>Descuento: <input type="hidden" name="descuento" id="descuento" value="0"></td>
		<td>Fletes: <input type="hidden" name="fletes" id="fletes" value="0"></td>
		<td>Retefuente: <input type="hidden" name="retefuente" id="retefuente" value="0" /></td>
	</tr>
	<tr>
		<td colspan="5"><label>OBSERVACIONES DEL AJUSTE</label><br><textarea id="observaciones" placeholder="Escriba las observaciones de este documento"></textarea></td>
	</tr>
</table>
...
function recorrerInventario(){
	...
	for(i=1 ; i<trs; i++){
		...
		cant = $('#cant'+i).val();
		precio = $('#precio'+i).val();
		totalproducto = totalproducto+':'+ref+','+nom+','+cant+','+precio+','+lot;
	}
...
function gruardarRemision(){
	...
	alert('productos'+fecha_remision);

	if(productos == '' || idremision == '' || nombre_cliente == '' || fecha_remision == '' || nit_cliente == '' || vendedor == '' || observaciones== ''){
    ...
    $.ajax({
            data:  parametros,
            url:   '../php/guardar/ajusteinventario.php',
            type:  'post',
            beforeSend: function () {
