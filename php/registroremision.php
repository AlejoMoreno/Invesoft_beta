...
$bill_producto = mysqli_query($conexion,"SELECT *, saldo as saldos FROM  `inventario` WHERE  `saldo` !=0 AND `idbodega` = '$bodegaactual' ORDER BY  `inventario`.`descripcion` ASC ") or die(mysqli_error($conexion)); 
...
<table class="tg">
  <tr>
    <th class="tg-031e" colspan="5">
		<div id="numeroremision"><label>Numero Remisión: </label><?php echo '<input type="text" name="idremision" id="idremision" value="'.$numdoc.'" disabled>';?></div>
    </th>
  </tr>
  <tr>
    ...
    <td class="tg-le8v" colspan="4" rowspan="2"><label>DATOS DEL PACIENTE :</label>
    	<table class="datos">
    		...
    		<tr>
    			...
    			<td><label>FECHA CIRUGIA :</label><input type="date" name="datosPaFec" id="datosPaFec" placeholder="Fecha Cirujia"></td>
    			<td></td>
    		</tr>
    	</table>
    </td>
...
function gruardarRemision(){
	...
	if(productos == '' || idremision == '' || nombre_cliente == '' || fecha_remision == '' || nit_cliente == '' || vendedor == '' || datosPaCed == '' || datosPaNPa == '' || datosPaNCi == '' || datosPaHis == '' || datosPaFec == '' || observaciones == ''){
